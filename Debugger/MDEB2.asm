...
_:
	...
	mov	byte ptr [bx], 0CBh ;code of retf in the begining of psp of loaded program (default: cd20), so when loaded program terminates it does retf, not ret
  	...
  	mov bx, psp+10h ;find a place where to write CC (breakpoint)
	mov cl, [bx + 0F5h] ;memorize original command (in current HW1 - jmp)
	mov [bx], cl 
	mov byte ptr [bx + 0F5h], 0CCh ;write CC instead of jmp.
